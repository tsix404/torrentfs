...
CREATE TABLE torrents (
    ...
    info_hash BLOB NOT NULL,                -- 种子 info_hash（20字节 SHA1）
    ...
    UNIQUE(info_hash, source_path)          -- 允许相同 info_hash 在不同 source_path 下存在
);
```
...
- `idx_torrents_info_hash_source_path` — 按复合键 (info_hash, source_path) 查找
