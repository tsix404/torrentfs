...
impl PieceCache {
    ...
    fn save_metadata(&self) -> TorrentResult<()> {
        let meta_path = self.metadata_path();
        let temp_path = meta_path.with_extension("meta.tmp");
        
        let mut file = File::create(&temp_path)
            .map_err(|e| TorrentError::IoError(format!("Failed to create temp metadata: {}", e)))?;
        ...
        file.sync_all()
            .map_err(|e| TorrentError::IoError(format!("Failed to sync metadata: {}", e)))?;
        
        fs::rename(&temp_path, &meta_path)
            .map_err(|e| TorrentError::IoError(format!("Failed to rename metadata: {}", e)))?;
